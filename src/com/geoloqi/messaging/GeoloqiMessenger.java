...
public class GeoloqiMessenger extends SQLiteOpenHelper implements Runnable {
	...
	public void run() {
		Util.log("Messenger is online.  ID is: "
	...
	private void sendData() {
		...
		while (first != null && running) {
			...
			boolean success = post.locationUpdate(context, message);
			//boolean success = (regulator.getWindowSize())/100.0 < 1.0; // debug
	...
	private void broadcastUnsentPointCount() {
		Uri uri = new Uri.Builder().scheme("mupdate").authority("geoloqi.com").path("/"
				+ getUnsentPointCount() + "/").build();
	...
	private class MessagingReceiver extends GeoloqiReceiver {
		...
		private boolean shouldSendData(Context context) {
			return lastSend < System.currentTimeMillis()
					- Util.getMinTime(context);
		}
	...
	public void onCreate(SQLiteDatabase db) {
		try {
			db.execSQL("CREATE TABLE lqLocationData (" + LocationListElement.ID
